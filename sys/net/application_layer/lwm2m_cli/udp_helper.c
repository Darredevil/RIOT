                       ...
                       uint8_t *data, size_t len)
{
    ...
    udp = ng_udp_hdr_build(payload, p, 2, p, 2);
    if (udp == NULL) {
    ...
    ip = ng_ipv6_hdr_build(udp, NULL, 0, (uint8_t *)addr, sizeof(ipv6_addr_t));
    if (ip == NULL) {
    ...
    /* send out the packet */
    ng_netapi_dispatch_send(NG_NETTYPE_UDP, NG_NETREG_DEMUX_CTX_ALL, ip);
    return len;
}
